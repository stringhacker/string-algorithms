...
\item wartości przy krawędziach zwykłych to są początkowe wartości z tablicy $\LCP$,
\item wartości przy krawędziach przerywanych to są wartości tablic $\LCP_{i}$.
\end{itemize}

Algorytm obliczający LPF będzie iterować tablice $\SA$ od lewej do prawej.
Trzy przypadki są do rozważenia w takim razie:

\begin{enumerate}
\item $\SA[i - 1] < \SA[i] > \SA[i + 1]$, czyli przypadek lokalnego maksimum.
  W tym przypadku, $\LPF[\SA[i]] = \max(\LCP[i], \LCP[i + 1])$.
  Nowa krawędź zostaje stworzona, zastępująca $\LCP[i], \LCP[i + 1]$, z wagą równą $\min(\LCP[i], \LCP[i + 1])$.
\end{enumerate}
